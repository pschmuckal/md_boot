...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case L_T_MD:
            if (record->event.pressed) {
                //Due to the Rocketeer's housing being completly opaque, only use the following modes
                //Note: Initial mode is set in rgb_matrix_init_user() in this file
...
void rgb_matrix_init_user(void) {
    led_lighting_mode = LED_MODE_KEYS_ONLY; //Start Rocketeer without backlighting
}
