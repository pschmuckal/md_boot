...
typedef struct issi3733_driver_s {
    ...
    uint8_t pwm_rdy[ISSI3733_PG_PWM_BYTES]; //PG1 - PWM Register ready for send
...
extern uint32_t power_sum;
extern uint32_t power_sum_last;
extern int8_t change_counter;
...
extern int8_t gcr_change_counter;              //GCR increase and decrease calls are counted here and acted upon when a count limit is hit;
